...
class Wumpus
{
    ...
    public function moveActual($ndx) {
        ...
        if($this->current->contains(self::WUMPUS,0)){
            return self::EATEN;
        }
        if($this->current->contains(self::PIT,0)){
            return self::FELL;
        }
        if($this->current->contains(self::BIRDS,0)){
            $this->carried = true;
    ...
    public function shoot($ndx) {
        ...
        $this->arrows--;
        $tempShotRoom = $this->rooms[$ndx];
        if($tempShotRoom->contains(self::WUMPUS)){
            return true;
        }
        $randInt = rand(0,2);
        $randRoom = $tempShotRoom->getNeighbors()[$randInt];
        if($randRoom->contains(self::WUMPUS)){
            return true;
        }
        return false;
